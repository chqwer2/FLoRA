#!/bin/bash
#SBATCH --job-name=gsm_stf
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=6:00:00
#SBATCH --array=0-3
#SBATCH --output=/mnt/scratch2/users/hchen/logs/gsm_stf_%A_%a.log
# Steerable LoRA REAL run (optimizer fix in place: steer now trains, ws!=0).
# capacity trend via dict size p; extreme rank r=1. eval@150 fast first read.
set -uo pipefail
source /mnt/scratch2/users/hchen/FLoRA/kelvin_env.sh
cd /mnt/scratch2/users/hchen/FLoRA/Experiments
#     r  p   k  seed
CFG=("2 8 2 1" "2 16 4 1" "2 32 4 1" "1 16 4 1")
read R P K SEED <<< "${CFG[$SLURM_ARRAY_TASK_ID]}"
A=$((2*R))
export LENA_STEER=1 LENA_STEER_P=$P LENA_STEER_K=$K
TAG=gsmSF_r${R}_p${P}k${K}_s${SEED}
OUT=/mnt/scratch2/users/hchen/FLoRA/Experiments/runs/$TAG
echo "[steerF] r=$R p=$P k=$K seed=$SEED"
