...
# ---------------- TwoPathTTT (verbatim: global causal SwiGLU inner-TTT + local causal conv) ----------------
...
# ---------------- fair first-#### extraction ----------------
def ln(s):
    n=re.findall(r"-?\d[\d,]*\.?\d*", str(s).replace(",","")); return n[-1].rstrip(".") if n else None
def norm(x):
    return None if x is None else str(x).replace(",","").strip().rstrip(".")
def extract(g):
...
def load_test(task,limit):
    ...
    if task=="gsm_symbolic":
        ...
        return o
    if task=="svamp":
        ds=load_dataset("ChilleD/SVAMP",split="test"); return [(PROMPT.format(q=(ds[i]["Body"].strip()+" "+ds[i]["Question"].strip())),norm(ds[i]["Answer"])) for i in range(min(limit,len(ds)))]
    if task=="asdiv":
        ds=load_dataset("EleutherAI/asdiv",split="validation"); return [(PROMPT.format(q=(ds[i]["body"].strip()+" "+ds[i]["question"].strip())),norm(ln(str(ds[i]["answer"])))) for i in range(min(limit,len(ds)))]
...
def train(args, model, tok, branches, device):
    ...
    step=0; steps=args.steps if args.steps>0 else (len(exs)//args.bs)*args.epochs; order=list(range(len(exs)))
    for ep in range(args.epochs):
        random.Random(args.seed+ep).shuffle(order)
        for bi in range(0,len(order),args.bs):
            idx=order[bi:bi+args.bs]; input_ids=[]; labels=[]
            for j in idx:
                p,a=exs[j]; pi=tok(p,add_special_tokens=True).input_ids; ai=tok(" "+a.strip(),add_special_tokens=False).input_ids+[tok.eos_token_id]
                input_ids.append((pi+ai)[:args.cutoff]); labels.append(([-100]*len(pi)+ai)[:args.cutoff])
            m=max(len(x) for x in input_ids)
            ii=torch.tensor([x+[tok.pad_token_id]*(m-len(x)) for x in input_ids],device=device); ll=torch.tensor([x+[-100]*(m-len(x)) for x in labels],device=device)
            out=model(input_ids=ii, attention_mask=(ii!=tok.pad_token_id).long(), labels=ll, use_cache=False)
            opt.zero_grad(); out.loss.backward(); gnorm=torch.nn.utils.clip_grad_norm_(branches.parameters(),1.0); opt.step(); step+=1
            if step%20==0 or step<=3:
                g=float(branches[0].gamma.detach().float()); print(f"step {step}/{steps} loss={float(out.loss):.4f} gnorm={float(gnorm):.3f} gamma0={g:.4f}",flush=True)
            if args.steps>0 and step>=args.steps: return

@torch.no_grad()
def evaluate(model, tok, device, task, limit, maxtok=128):
    model.eval(); test=load_test(task,limit); c=0
    for p,gold in test:
        ids=tok(p,return_tensors="pt").input_ids.to(device)   # bs=1: no padding => branch prefix-state correct
    ...
    acc=c/len(test); print(f"[EVAL] {task} exact_match={acc:.4f} n={len(test)}",flush=True); return acc

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--base_model",default="meta-llama/Llama-2-7b-hf"); ap.add_argument("--r",type=int,default=64)
    ap.add_argument("--targets",default="attn"); ap.add_argument("--n_train",type=int,default=7000)
    ...
    ap.add_argument("--seed",type=int,default=1); ap.add_argument("--out",required=True); ap.add_argument("--mode",default="train_eval")
    ap.add_argument("--eval_limit",type=int,default=200); ap.add_argument("--eval_tasks",default="gsm8k,gsm_symbolic")
    args=ap.parse_args(); dev='cuda'; targets=[t.strip() for t in args.targets.split(",")]
    ...
    branches=build(model, args.r, targets, dtype=torch.float32, device=dev)
    ntr=sum(p.numel() for p in branches.parameters())
    print(f"[BUILD] targets={targets} branches={len(branches)} r={args.r} trainable={ntr:,} ({100*ntr/sum(p.numel() for p in model.parameters()):.3f}%)",flush=True)
    if args.mode=="eval":
        branches.load_state_dict(torch.load(args.out+"/branches.pt",map_location=dev))
        for t in args.eval_tasks.split(","): evaluate(model,tok,dev,t.strip(),args.eval_limit); return
    if args.mode=="smoke": args.steps=5; args.n_train=64
    train(args, model, tok, branches, dev)
    os.makedirs(args.out, exist_ok=True); torch.save(branches.state_dict(), args.out+"/branches.pt")
    print(f"[SAVE] {args.out}/branches.pt",flush=True)
    if args.mode=="smoke":
        branches.load_state_dict(torch.load(args.out+"/branches.pt",map_location=dev)); print(f"[SMOKE_OK] gamma0={float(branches[0].gamma):.5f}",flush=True); return
    for t in args.eval_tasks.split(","): evaluate(model,tok,dev,t.strip(),args.eval_limit)
