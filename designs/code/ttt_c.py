...
# C-design: LoRA (static, handles in-dist) + "do-no-harm" gated TTT branch (per-channel gate + L1; learnable inner_lr).
class TwoPathTTT(nn.Module):
    def __init__(self, d_in, d_out, r, ksize=4):
        ...
        self.ilr=nn.Parameter(torch.tensor(1.0))
...
class TTTBranch(nn.Module):
    def __init__(self,d,r): super().__init__(); self.ttt=TwoPathTTT(d,d,r); self.gate=nn.Parameter(torch.ones(d))  # per-channel gate
    def forward(self,h): return (self.gate*self.ttt(h)).to(h.dtype)
...
def train(a,model,tok,branches,dev):
    ...
    for ep in range(a.epochs):
        ...
        for bi in range(0,len(order),a.bs):
            ...
            l1=sum(b.gate.abs().mean() for b in branches)/len(branches)
            loss=out.loss + a.l1*l1
            opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(params,1.0); opt.step(); step+=1
            if step%20==0 or step<=3:
                gmean=float(torch.stack([b.gate.abs().mean() for b in branches]).mean()); il=float(branches[0].ttt.ilr)
                print(f"step {step}/{steps} loss={float(out.loss):.4f} l1={float(l1):.3f} gate|mean|={gmean:.3f} ilr0={il:.3f}",flush=True)
...
def main():
    ...
    ap.add_argument("--lora",action="store_true"); ap.add_argument("--lora_r",type=int,default=8); ap.add_argument("--l1",type=float,default=1e-4)
    ...
    ap.add_argument("--out",required=True); ap.add_argument("--mode",default="train_eval"); ap.add_argument("--eval_limit",type=int,default=150); ap.add_argument("--eval_tasks",default="gsm8k,gsm_symbolic")
    ...
    print(f"[BUILD-C] lora={a.lora} r={a.r} l1={a.l1} trainable={ntr:,}",flush=True)
    if a.mode=="smoke": a.n_train=64; a.epochs=1
    if a.mode=="smoke":
        exs=gsm_examples(64)
        for n,p in model.named_parameters(): p.requires_grad_("lora_" in n)
        for p in branches.parameters(): p.requires_grad_(True)
        ...
        for st in range(5):
            ...
            o=model(input_ids=ids,labels=lab,use_cache=False); l1=sum(b.gate.abs().mean() for b in branches)/len(branches)
            (o.loss+a.l1*l1).backward(); opt.step(); opt.zero_grad()
            print(f"smoke {st} loss={float(o.loss):.3f} gate|mean|={float(torch.stack([b.gate.abs().mean() for b in branches]).mean()):.4f}",flush=True)
